...
if 'current_theme' in st.session_state:
    ...
    if st.button("Submit Haiku"):
        if user_haiku:
            # First API call: Evaluate the user's haiku
            evaluation_prompt = f"""Evaluate this haiku: "{user_haiku}"
            
            Check if it follows the 5-7-5 syllable pattern. Give a brief, encouraging response that says whether it's a proper haiku or not, and maybe one tip if it needs improvement."""
